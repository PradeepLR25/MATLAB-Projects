...
av_files = dir(fullfile('C:','Documents and Settings','Pradeep','Desktop','ndAnalysis','q99a5cALL','*.pdb'))
...
for load_txt=1:75
    ...
    str=['q99a5cHDPB2_cns_w_OUT_',num2str(load_txt),'.pdb'];
    %str20=['C:/Documents and Settings\Pradeep\Desktop\2ndAnalysis\q99a5cNOCA\spPBproper4_cns_w_',num2str(load_txt),'.pdb']
    search=strcmp(ST2Cell, str);
    if sum(sum(search))==1
        
        [atom, atmnmb, atmname, resname, resnum, x, y, z, occ, bfac, id, atnm]= textread(str, '%s%d%s%s%f%f%f%f%f%f%s%s', 'headerlines',60);
        i=1;
        l=1;
...
%DRG3(var,:)=sum(DRG3)/75
%DRG4(var,:)=sum(DRG4)/75
%DRG5(var,:)=sum(DRG5)/75
%DRG6(var,:)=sum(DRG6)/75
%DRG7(var,:)=sum(DRG7)/75
%DRG8(var,:)=sum(DRG8)/75
end
fclose(fid)
fclose(fid2)